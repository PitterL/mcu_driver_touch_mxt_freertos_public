 ...
 */
#define DEFAULT_TASK_STACK_DEPTH 500
#define ASYNC_TASK_STACK_DEPTH 500
#define DEFAULT_TASK_PRIORITY (configTIMER_TASK_PRIORITY + 2)
#define ASYNC_TASK_PRIORITY (configTIMER_TASK_PRIORITY + 1)
...
#define kthread_run2(__threadfn, __data, __name, __stack_size, __priority)			   \
({									   \
	task_handle_p __k;						   \
	portBASE_TYPE __result		\
		= xTaskCreate(__threadfn, __name, __stack_size, __data, __priority, &__k); \
	if (__result != pdTRUE)						   \
		__k = NULL;					   \
	__k;								   \
})
