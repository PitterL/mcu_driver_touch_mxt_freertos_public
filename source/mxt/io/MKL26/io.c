...
void device_regulator_enable(struct device *dev)
{
	...
	device_wait_irq_state(dev, 0, MXT_RESET_TIME);
}
...
int device_gpio_configure(struct device *dev)
{
	...
	if (gpio_is_valid(pdata->gpio_reset)) {
		...
		gpio_request(pdata->gpio_reset, "mxt_reset_gpio");
	...
	if (gpio_is_valid(pdata->gpio_irq)) {
		...
		gpio_request(pdata->gpio_irq, "mxt_irq_gpio");
		gpio_set_pin_direction(pdata->gpio_irq, GPIO_DIRECTION_IN);
		//may set pullup ...
		gpio_set_pin_pull_mode(pdata->gpio_irq, GPIO_PULL_UP);
	} else {
...
{
	assert(false); //inq handler is register by system
}

void BOARD_CHG_IRQ_HANDLER(void)
{
	...
	pdata->irq_fn(pdata->irq, dev_id);

	ext_irq_clear(pdata->gpio_irq);
}
			...
			irq_handler_t thread_fn, const char *devname, void *dev_id)
{
	...
	ret = ext_irq_register(pdata->irq, pdata->gpio_irq, irq_cb_handler);
...
static bool device_parse_platform(struct device *dev)
{
	...
	pdata->gpio_irq = GPIO(BOARD_CHG_GPIO_ID, BOARD_CHG_GPIO_PIN);	//----- here write irq io number
	//transfer to IRQ
	pdata->irq = BOARD_CHG_IRQ/*gpio_to_irq(pdata->gpio_irq)*/;

	pdata->irqflags = IRQF_TRIGGER_LOW;

	pdata->gpio_reset = GPIO(BOARD_RST_GPIO_ID, BOARD_RST_GPIO_PIN);  //---- here write reset io number
