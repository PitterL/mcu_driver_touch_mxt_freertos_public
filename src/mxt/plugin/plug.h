...
#define PL_STATUS_FLAG_PLUG_FIG					(1<< (PLUGIN_FIG + PL_STATUS_FLAG_HIGH_MASK_SHIFT))
...
struct plugin_device{
	...
	void (*pre_process)(struct plugin_device *p, unsigned long pl_flag);
	long (*post_process)(struct plugin_device *p, unsigned long pl_flag);
