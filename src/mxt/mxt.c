...
#define MXT_MAX_BLOCK_WRITE	64
#define MXT_DMA_BUFFER_SIZE 64		//must algn for DMA use
...
extern void mxt_plugin_pre_process(struct plug_interface *pl, bool in_boot);
extern long mxt_plugin_post_process(struct plug_interface *pl, bool in_boot);
...
extern int mxt_plugin_init(struct plug_interface *pl);
				...
				void *val, unsigned long flag)
{
	...
	w_cache = (u8 *)data->i2c_dma_va + (data->dma_buffer_size >> 1);
	w_cache_pa = (u8 *)data->i2c_dma_pa + (data->dma_buffer_size >> 1);
#else
...
static int mxt_plugin_pre_init(struct mxt_data *data)
{
	...
	pl->priv_data = data;
	pl->dev0 = dev;
	error = mxt_get_init_cfg(pl, data->max_x, data->max_y, data->info->matrix_xsize, data->info->matrix_ysize);
	if (error) {
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	error = mxt_plugin_init(&data->plug);
	if (error) {
		dev_err(&client->dev, "Error %d plugin init\n",
			error);
		return error;
